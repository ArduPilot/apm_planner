...
void CameraGimbalConfig::addOutputRcChannels(QComboBox* comboBox)
{
    ...
    comboBox->addItem("SERVO5", 5);
    comboBox->addItem("SERVO6", 6);
    comboBox->addItem("SERVO7", 7);
    comboBox->addItem("SERVO8", 8);
    comboBox->addItem("SERVO9", 9);
    comboBox->addItem("SERVO10", 10);
    comboBox->addItem("SERVO11", 11);
    comboBox->addItem("SERVO12", 12);
    comboBox->addItem("SERVO13", 13);
    comboBox->addItem("SERVO14", 14);
    comboBox->addItem("SERVO15", 15);
    comboBox->addItem("SERVO16", 16);
}
                                        ...
                                        QSpinBox* angleMin, QSpinBox* angleMax, QCheckBox* stabilize)
{
    ...
    pm->setParameter(1, "SERVO" + channel + "_FUNCTION", rcFunction);
    pm->setParameter(1, "SERVO" + channel + "_MIN", servoMin->value());
    pm->setParameter(1, "SERVO" + channel + "_MAX", servoMax->value());

    if(servoReverse->checkState() == Qt::Checked){
        pm->setParameter(1, "SERVO" + channel + "_REVERSED", 1.0);
    } else {
        pm->setParameter(1, "SERVO" + channel + "_REVERSED", 0.0);
    }
    ...
    pm->setParameter(1, "MNT_DEFLT_MODE", QVariant(3)); // Set the mount mode to RC Targetting
}
...
void CameraGimbalConfig::refreshMountParameters(QString mount, QString parameterName, QVariant &value)
{
    ...
    } else if (parameterName == "MNT_DEFLT_MODE") {
...
void CameraGimbalConfig::parameterChanged(int uas, int component, QString parameterName, QVariant value)
{
    ...
    if (parameterName.startsWith("MNT")){
        ...
        else if (parameterName.endsWith("REVERSED"))
        {
            if (value.toInt() == 1)
            {
...
void CameraGimbalConfig::updateMountMode(int index)
{
    if(m_uas){
        ...
        if(pm){
            pm->setParameter(1, "MNT_DEFLT_MODE", QVariant(static_cast<int>(mode))); // Set the mount mode to RC Targetting
        }
    }
}
