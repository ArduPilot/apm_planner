...
REPO_URL=https://github.com/ArduPilot/apm_planner

RPMBUILD_ROOT="${HOME}/rpmbuild"

if [[ -f ../apm_planner.pro ]]
then
    WORK_PATH=..
elif [[ -f apm_planner.pro ]]
then
    WORK_PATH=.
else
...
tar --exclude-vcs --exclude=Drivers --exclude=avrdude --exclude=uploader --xform=s%^${WORK_PATH_REGEX}%${PROJECT_NAME}-${VERSION}/% -z -c -f ${TARBALL_PATH} ${WORK_PATH}
echo "done";

echo -n "Copying patches ... "
cp ${WORK_PATH}/redhat/apmplanner2-*.patch ${RPMBUILD_ROOT}/SOURCES/
echo "done"
...
echo -n "Copying ardupilot.repo ... "
REPO_FILE="${WORK_PATH}/redhat/ardupilot.repo"
cp "${REPO_FILE}" "${RPMBUILD_ROOT}/SOURCES"
echo "done"

echo -n "Copying ardupilot-repo.spec ... "
OLD_SPEC="${WORK_PATH}/redhat/ardupilot-repo.spec"
