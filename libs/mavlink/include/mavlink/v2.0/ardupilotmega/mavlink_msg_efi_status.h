...
#define MAVLINK_MESSAGE_INFO_EFI_STATUS { \
    ...
    {  { "health", NULL, MAVLINK_TYPE_UINT8_T, 0, 52, offsetof(mavlink_efi_status_t, health) }, \
         ...
         { "exhaust_gas_temperature", NULL, MAVLINK_TYPE_FLOAT, 0, 53, offsetof(mavlink_efi_status_t, exhaust_gas_temperature) }, \
         { "throttle_out", NULL, MAVLINK_TYPE_FLOAT, 0, 57, offsetof(mavlink_efi_status_t, throttle_out) }, \
         { "pt_compensation", NULL, MAVLINK_TYPE_FLOAT, 0, 61, offsetof(mavlink_efi_status_t, pt_compensation) }, \
         } \
}
#else
#define MAVLINK_MESSAGE_INFO_EFI_STATUS { \
 ...
 * @param cylinder_head_temperature [degC] cylinder_head_temperature
                               ...
                               uint8_t health, float ecu_index, float rpm, float fuel_consumed, float fuel_flow, float engine_load, float throttle_position, float spark_dwell_time, float barometric_pressure, float intake_manifold_pressure, float intake_manifold_temperature, float cylinder_head_temperature, float ignition_timing, float injection_time, float exhaust_gas_temperature, float throttle_out, float pt_compensation)
{
    ...
    _mav_put_float(buf, 53, exhaust_gas_temperature);
    _mav_put_float(buf, 57, throttle_out);
    _mav_put_float(buf, 61, pt_compensation);
 ...
 *
 * @return [degC] cylinder_head_temperature
 */
